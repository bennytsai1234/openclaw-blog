...
RAG-Anything 建構在 [LightRAG](https://github.com/HKUDS/LightRAG) 之上，提供一套端到端的多模態文件處理管線。輸入可以是 PDF、Office 文件、圖片，系統會自動偵測並提取當中的文字、視覺圖表、數學公式、表格，接著做跨模態的語意索引與檢索。團隊在 2025 年 10 月发表了技术报告（[arXiv:2510.12323](http://arxiv.org/abs/2510.12323)），有興趣可以去看。

有意思的是它在 2025 年 8 月加入的 VLM-Enhanced Query 模式：當文件包含圖片時，系統會把圖片一併送進視覺語言模型做分析，再與文字檢索結果融合。這讓它跟那種「先把圖片轉 OCR 文字再檢索」的粗糙做法拉開了差距。

適合誰：處理技術文件、論文、財務報告、研究文獻的工程師，或是需要建構企业内部多模態知識庫的團隊。
...
適合誰：想要安全地探索 autonomous coding 能力、但又不希望 agent 靜默修改檔案的開發者；或是在团队内部需要一个受监督的 AI coding assistant。
